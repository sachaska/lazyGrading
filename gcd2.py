...
class GroupCoordinatorDaemon(socketserver.BaseRequestHandler):
    """
    A Group Coordinator Daemon (GCD) which will respond with a list of potential group members to a text message COMMENCE
    ...
    listeners_by_pid = {}  # listener address indexed by process id (as returned from COMMENCE message)
    ...
    def handle(self):
        """
        Handles the incoming messages - expects only GCD_MESSAGEs
        """
    ...
    def handle_join(message):
        """
        Process this GCD_MESSAGE by adding the new member into the group
        data structures.
        Also do some validation:
        - of the right form
        - listener is on localhost (or equivalent)

        :param message: (GCD_MESSAGE, ((days_to_bd, su_id), (host, port)))
        ...
        if message_name != GroupCoordinatorDaemon.GCD_MESSAGE:
